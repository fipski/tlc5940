...
#define TLC5940_DEVICES                 1               // num of devices
#define TLC5940_LED_NAME_SZ				16              // name length
...
#define DEVICE_NAME "fbuf"                              // SYSFS device name in /dev/
#define CLASS_NAME "tlc_framebuffer"


/* 
 * (some) Prototypes for sysfs
 */
...
bool    new_data;
bool    latching;
struct  kobject *kobj_ref;                            //TODO cekck if necessary
...
static enum hrtimer_restart tlc5940_timer(struct hrtimer *timer)
{
    ...
    /* toggle blank pin to reset tlc5940's GS Counter */
...
static void tlc5940_work(struct work_struct *work)
{
    ...
    gpio_set_value(tlc->blank_gpio, 1); // recommended in datasheet
        ...
        const enum led_brightness brightness)
{
    ...
    if (led->id % 2){
        ...
        framebuffer_tx[led->id *3/2] = brightness >> 4; 
...
static int tlc5940_probe(struct spi_device *spi)
{
    ...
    for (count = 0; count < tlcdev->chain_sz; count++) {
        ...
        for (i = 0; i < TLC5940_LEDS; i++) {
            ...
            leddev->ldev.max_brightness = 0xfff; //TODO add GS resolution here, 3ff for 10bit
